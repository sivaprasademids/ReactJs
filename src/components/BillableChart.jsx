...
// Chart Component
const BillableChart = ({ entries })=> {
  const billableHours = entries.filter(e => e.billable).reduce((sum, e) => sum + e.duration, 0);
  const nonBillableHours = entries.filter(e => !e.billable).reduce((sum, e) => sum + e.duration, 0);
  ...
  return (
    <div className="mt-6 p-2 bg-white shadow rounded-xl w-full md:w-1/2 h-80">
      ...
      <ResponsiveContainer width="100%" height="100%">
        <PieChart>
          <Pie data={data} innerRadius={30} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={80} fill="#8884d8" label>
